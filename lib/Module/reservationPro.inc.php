...
<form method="POST" id="Reservation" name="Reservation" action="<?php echo $Home; ?>/Reservation-Professionnel/">
    <div id="Groupe">
        Départ<BR />
    ...
    <div id="Groupe">
        ...
        <select class="Module" name="destination" required>
            <?php 
            $SelectDestination=$cnx->prepare("SELECT * FROM ".$Prefix."neuro_DestinationPro");
            ...
            while ($destination=$SelectDestination->fetch(PDO::FETCH_OBJ)) { ?>
                <option value="<?php echo $destination->libele; ?>" <?php if ((isset($_SESSION['destinationPro']))&&($_SESSION['destinationPro']==$destination->libele)) { echo "selected"; } ?> ><?php echo $destination->libele; ?></option>
            <?php } ?>
        </select>
    </div><BR /><BR />
    ...
    <div id="Groupe">
        ...
        <select class="Module" name="destinationRetour">
            <?php 
            $SelectDestinationRetour=$cnx->prepare("SELECT * FROM ".$Prefix."neuro_DestinationPro");
            $SelectDestinationRetour->execute();

            echo '<option value="">-- Indiquez une destination --</option>';
            while ($destinationRetour=$SelectDestinationRetour->fetch(PDO::FETCH_OBJ)) { ?>
                <option value="<?php echo $destinationRetour->libele; ?>" <?php if ((isset($_SESSION['destinationRetourPro']))&&($_SESSION['destinationRetourPro']==$destinationRetour->libele)) { echo "selected"; } ?> ><?php echo $destinationRetour->libele; ?></option>
            <?php } ?>
        </select>
    </div><BR />
