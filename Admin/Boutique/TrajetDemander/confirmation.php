...
if (isset($_POST['Confirmer'])) {
    ...
    if ($Confirmation==3) {
        ...
        if ((!$Update)||(!$InsertRefu)) {
            ...
            ErreurLog($Erreur);
            header("location:".$Home."/Admin/Boutique/TrajetDemander/?erreur=".urlencode($Erreur));
        }
        else {
            ...
            if (EnvoiNotification($Societe, $Serveur, $Destinataire, "illicab - Confirmation de trajet", $Body, $Contact->email)==false) {
            ...
            if ((!$Update)||(!$InsertValid)) {
                ...
                if ($Confirmation==$Trajet->etat) {
                    ...
                    if (EnvoiNotification($Societe, $Serveur, $Destinataire, "illicab - Modification du trajet n°: ".$Id, $Body, $Contact->email)==false) {
                        ...
                        header("location:".$Home."/Admin/Boutique/TrajetValider/?valid=".urlencode($Valid)."#".$Id);
                    }
                }
                else {
                    ...
                    if (EnvoiNotification($Societe, $Serveur, $Destinataire, "illicab - Votre trajet n°: ".$Id, $Body, $Contact->email)==false) {
                        $Erreur="L'e-mail n'a pu être envoyé, Veuillez vérifier l'adresse email du client ! </p>";
                        ErreurLog($Erreur);
                        header("location:".$Home."/Admin/Boutique/TrajetValider/?erreur=".urlencode($Erreur));
                    } 
