...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $deleteActu=$cnx->prepare("DELETE FROM ".$Prefix."neuro_Destination WHERE id=:id");
    $deleteActu->bindParam(':id', $Id, PDO::PARAM_INT);
    $deleteActu->execute();

    header('location:'.$Home.'/Admin/Boutique/Destination/');
}
...
Etes-vous sur de vouloir supprimer cette destination ? </p>
