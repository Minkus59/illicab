...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $deleteActu=$cnx->prepare("DELETE FROM ".$Prefix."neuro_Planning WHERE id=:id");
    $deleteActu->bindParam(':id', $Id, PDO::PARAM_INT);
    $deleteActu->execute();

    header('location:'.$Home.'/Admin/Boutique/Planning/Gestion/');
}

if ((!empty($_GET['id']))&&(isset($_POST['non']))) {  
    header('location:'.$Home.'/Admin/Boutique/Gestion/');
}
?>  
...
Etes-vous sur de vouloir supprimer ce planning ? </p>
