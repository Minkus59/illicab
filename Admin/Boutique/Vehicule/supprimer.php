...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $deleteActu=$cnx->prepare("DELETE FROM ".$Prefix."neuro_Vehicule WHERE id=:id");
    $deleteActu->bindParam(':id', $Id, PDO::PARAM_INT);
    $deleteActu->execute();

    header('location:'.$Home.'/Admin/Boutique/Vehicule/');
}
...
Etes-vous sur de vouloir supprimer ce véhicule ? </p>
